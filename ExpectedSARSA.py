...
class EpsilonGreedyTabularExpectedSARSA():
    """
    ...
    def update_action_values(self,
        ...
        for i in range(num_actions):
            if i == best_action:
                expected_value += ((1 - self.epsilon) + (self.epsilon / num_actions)) * self.q[next_state][i]
            else:
                expected_value += (self.epsilon / num_actions) * self.q[next_state][i]
        self.q[state][action] += alpha * (reward + \
                                          self.gamma * expected_value - self.q[state][action])
    ...
    def play_step(self,
                 state, action, epsilon, alpha):
        """
        Plays one step without updating action-values.
    ...
    def play(self,
                    epsilon, alpha, decay_alpha, decay_epsilon):
        """
        Plays a single episode of the cartpole task.
